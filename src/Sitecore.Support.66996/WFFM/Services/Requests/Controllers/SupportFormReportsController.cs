...
namespace Sitecore.Support.WFFM.Services.Requests.Controllers
{
    ...
    public class SupportFormReportsController : Controller
    {
        ...
        public SupportFormReportsController()
        ...
        public ActionResult GetFormContactsPage(Guid id, PageCriteria pageCriteria, string sortCriteria)
        {
            ...
            IEnumerable<IFormContactsResult> formsStatisticsByContact =
                this.FormsDataProvider.GetFormsStatisticsByContact(id, pageCriteria);
        ...
        public ActionResult GetFormSummary(Guid id)
        {
            ...
            string displayName = itemFromMasterDatabase.DisplayName;
        ...
        {
            IEnumerable<IFormFieldStatistics> formFieldsStatistics =
                this.FormsDataProvider.GetFormFieldsStatistics(id);
            foreach (IFormFieldStatistics current in formFieldsStatistics)
            {
                ...
                if (itemFromMasterDatabase != null)
                {
                    IFieldItem fieldItem = this.itemRepository.CreateFieldItem(itemFromMasterDatabase);
                    if (fieldItem.ParametersDictionary.ContainsKey("islist"))
                    {
                        current.IsList = true;
                    }
                }
